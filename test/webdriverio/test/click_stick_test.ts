...
async function exitMoveMode(browser: Browser, method: 'enter' | 'escape' | 'click' | 'tap' = 'enter'): Promise<void> {
  if (method === 'enter') {
    ...
    const workspace = await browser.$('.blocklySvg');
    await tapElement(browser, workspace);
    await browser.pause(PAUSE_TIME);
  }
}
...
suite('Touch-based Click-n-Stick Tests', function () {
  ...
  suite('Tap connection to move block', function () {
    ...
    test('Tapping invalid connection does not exit move mode', async function () {
      ...
      // Should have exited move mode (tap on workspace releases the block, wait for state to clear)
      await waitForDraggingToStop(this.browser);
      await waitForStickyModeToExit(this.browser);
    });
  });
...
// (tapBlock, tapElement, and inline workspace taps) include a click event.
//
...
async function tapElement(browser: Browser, element: any): Promise<void> {
  ...
  await browser.execute((attrs: {id: string, class: string}) => {
    ...
    // Synthesize a click event after the touch sequence, mimicking browser behavior
