...
suite('Click and Stick Move Mode Tests', function () {
  ...
  suite('Arrow key navigation in move mode', function () {
    test('Arrow keys navigate between connection candidates', async function () {
      ...
      chai.assert.exists(
        initialCandidate,
        'Should have initial connection candidate',
      );
      ...
      chai.assert.notDeepEqual(
        candidate1,
        initialCandidate,
        'Arrow right should change candidate',
      );
      chai.assert.notDeepEqual(
        candidate2,
        candidate1,
        'Second arrow right should change candidate',
      );
      ...
      chai.assert.deepEqual(
        candidateLeft,
        candidate1,
        'Arrow left should return to previous candidate',
      );
    ...
    test('Down and up arrows navigate connection candidates', async function () {
      ...
      chai.assert.notDeepEqual(
        candidateDown,
        initialCandidate,
        'Arrow down should change candidate',
      );
      ...
      chai.assert.deepEqual(
        candidateUp,
        initialCandidate,
        'Arrow up should return to initial candidate',
      );
  ...
  suite('Enter key to confirm move', function () {
    test('Enter key completes move to connection candidate', async function () {
      ...
      chai.assert.exists(
        connectionCandidate,
        'Should have connection candidate',
      );
      ...
      chai.assert.notDeepEqual(
        initialInfo,
        finalInfo,
        'Block should be in new position after move',
      );
    });
    ...
    test('Enter key without connection candidate stops move', async function () {
      ...
      chai.assert.isNull(
        candidate,
        'Should have no connection candidate in unconstrained position',
      );
    ...
    test('Enter key returns block to original position when no move made', async function () {
      ...
      await focusOnBlock(this.browser, 'draw_circle_1');
      const initialPosition = await getBlockPosition(
        this.browser,
        'draw_circle_1',
      );
      const initialInfo = await getFocusedNeighbourInfo(this.browser);
      ...
      chai.assert.isTrue(
        await isInStickyMode(this.browser),
        'Should be in sticky mode after double-click',
      );
      chai.assert.strictEqual(
        await getStickyBlockId(this.browser),
        'draw_circle_1',
        'Correct block should be in sticky mode',
      );
      chai.assert.isTrue(await isDragging(this.browser));
      ...
      chai.assert.isFalse(
        await isInStickyMode(this.browser),
        'Should exit sticky mode after Enter',
      );
      chai.assert.isFalse(await isDragging(this.browser));

      // Block should be in exact same position
      const finalPosition = await getBlockPosition(
        this.browser,
        'draw_circle_1',
      );
      const finalInfo = await getFocusedNeighbourInfo(this.browser);
      ...
      chai.assert.deepEqual(
        initialInfo,
        finalInfo,
        'Block connections should be unchanged',
      );
    });
  });
  ...
  suite('Click on destination connection point', function () {
    test('Click on connection point moves block there', async function () {
      ...
      const targetConnection = await findConnectionPoint(
        this.browser,
        'draw_circle_1',
      );
      chai.assert.exists(
        targetConnection,
        'Should find target connection point',
      );
      ...
      const blockPosition = await getBlockConnectionInfo(
        this.browser,
        'draw_circle_1',
      );
      chai.assert.exists(
        blockPosition.parentId,
        'Block should now have a parent',
      );
    });
    ...
    test('Click on incompatible connection does not move block', async function () {
      ...
      const valueConnection = await findValueConnectionPoint(
        this.browser,
        'value_1',
      );
      if (valueConnection) {
        ...
        if (stillDragging) {
          ...
          chai.assert.deepEqual(
            initialInfo,
            finalInfo,
            'Block should not move to incompatible connection',
          );
        }
      }
    });
  ...
  suite('Mouse movement with block following', function () {
    test('Moving mouse in move mode makes block follow cursor', async function () {
      ...
      chai.assert.notDeepEqual(
        initialPosition,
        newPosition,
        'Block should follow mouse movement',
      );
      ...
      chai.assert.notDeepEqual(
        newPosition,
        finalPosition,
        'Block should continue following mouse',
      );
    ...
    test('Mouse movement opens spaces for block placement', async function () {
      ...
      if (markerExists) {
        chai.assert.isTrue(
          markerExists,
          'Insertion marker should appear when hovering near blocks',
        );
      }
  ...
  suite('Single click to release block', function () {
    ...
    test('Double-click block, move mouse to empty space, single click drops block there', async function () {
      ...
      chai.assert.isTrue(
        await isInStickyMode(this.browser),
        'Should remain in sticky mode during mouse movement',
      );

      // Verify block has followed mouse movement
      const movedPosition = await getBlockPosition(
        this.browser,
        'draw_circle_1',
      );
      chai.assert.notDeepEqual(
      ...
      chai.assert.isFalse(
        await isInStickyMode(this.browser),
        'Should exit sticky mode after click',
      );
      chai.assert.isFalse(await isDragging(this.browser));
      ...
      chai.assert.deepEqual(
        movedPosition,
        finalPosition,
        'Block should stay where it was dropped',
      );
      chai.assert.notDeepEqual(
        initialPosition,
        finalPosition,
        'Block should not return to original position',
      );
    });
  ...
  suite('Connection Highlights Cleanup Tests', function () {
    test('Connection highlights are cleared after completing move with Enter', async function () {
      ...
      const manualStartDragResult = await this.browser.execute(() => {
        ...
        if (
          dragStrategy &&
          dragStrategy.constructor.name === 'KeyboardDragStrategy'
        ) {
          try {
            ...
            return {
              success: true,
              highlightedElementsAfter: dragStrategy.connectionHighlighter
                ? dragStrategy.connectionHighlighter.highlightedElements?.size
                : 0,
      ...
      chai.assert.isAbove(
        highlightCount,
        0,
        'Should have connection highlights during move',
      );
      ...
      chai.assert.equal(
        finalCount,
        0,
        'Connection highlights should be cleared after completing move',
      );
    });
    ...
    test('Connection highlights are cleared after completing move by clicking on workspace', async function () {
      ...
      chai.assert.equal(
        finalCount,
        0,
        'Connection highlights should be cleared after clicking on workspace',
      );
    });
    ...
    test('Connection highlights are cleared after completing move by clicking on connection', async function () {
      ...
      const highlightElement = await this.browser.$(
      ...
      if (await highlightElement.isExisting()) {
        ...
        chai.assert.equal(
          finalCount,
          0,
          'Connection highlights should be cleared after clicking on connection',
        );
      }
    });
    ...
    test('Connection highlights are cleared after canceling move with Escape', async function () {
      ...
      chai.assert.equal(
        finalCount,
        0,
        'Connection highlights should be cleared after canceling move with Escape',
      );
    });
    ...
    test('Connection highlights persist during STATEMENT block movement and stack re-computation', async function () {
      ...
      chai.assert.isAbove(
        initialCount,
        0,
        'Should have connection highlights initially',
      );
      ...
      const countAfterMovement = await getConnectionHighlightCount(
        this.browser,
      );
      chai.assert.isAbove(
        countAfterMovement,
        0,
        'Connection highlights should persist during block movement',
      );

      // Verify we have roughly the same number of highlights (within a reasonable range)
      const highlightCountDifference = Math.abs(
        countAfterMovement - initialCount,
      );
      chai.assert.isBelow(
        highlightCountDifference,
        3,
        'Should have roughly the same number of highlights',
      );
    ...
    test('Connection highlights are cleared after simulated touch interaction - double click to start, single click to drop', async function () {
      ...
      chai.assert.isTrue(
        ...
        'Should be dragging after double-click',
      );
      ...
      chai.assert.isFalse(
        await isInStickyMode(this.browser),
        'Should exit sticky mode after single click',
      );
      chai.assert.isFalse(
        await isDragging(this.browser),
        'Should not be dragging after single click',
      );
      ...
      chai.assert.equal(
        finalCount,
        0,
        'Connection highlights should be cleared after click drop',
      );
  ...
  suite('Regression Test - JavaScript Error Detection', function () {
    test('Double-click, Enter should not cause JavaScript errors', async function () {
      ...
      await assertNoJavaScriptErrors(
        this.browser,
        'double-click and Enter key handling',
      );

      // Verify the operation completed successfully (block should not be dragging)
      chai.assert.isFalse(
        await isDragging(this.browser),
        'Block should have been dropped successfully',
      );
    });
  });
  ...
  suite('Escape key cancels move mode', function () {
    test('Double-click block, move mouse, Esc cancels and returns to original position', async function () {
      ...
      await assertNoJavaScriptErrors(
        this.browser,
        'Escape key handling in move mode',
      );

      // Should no longer be in sticky mode or dragging
      chai.assert.isFalse(
        await isInStickyMode(this.browser),
        'Should exit sticky mode after Escape',
      );
      chai.assert.isFalse(await isDragging(this.browser));
      ...
      chai.assert.deepEqual(
        initialInfo,
        finalInfo,
        'Block connections should be restored',
      );
    });
  });

  suite(
    'Control: Mouse click selection should work after click-and-stick',
    function () {
      test('Single click should select block after mouse click-and-stick to workspace (SHOULD PASS)', async function () {
        ...
        chai.assert.isFalse(
          await isInStickyMode(this.browser),
          'Should have exited sticky mode',
        );
...
async function findValueConnectionPoint(browser: Browser, blockId: string) {
  ...
  const connections = await browser.$$(
    `[data-id="${blockId}"] .blocklyConnection`,
  );
...
function getStickyBlockId(browser: Browser): Promise<string | null> {
  return browser.execute(() => {
    // Access the KeyboardNavigation instance through the global
    const keyboardNav = (window as any).keyboardNavigation;
    return keyboardNav && keyboardNav.stickyBlock
      ? keyboardNav.stickyBlock.id
      : null;
  });
}
 ...
 */
async function realDoubleClickBlock(
  browser: Browser,
  blockId: string,
): Promise<void> {
  await browser.execute((blockId) => {
    ...
    console.log(
      `Dispatching real double-click on block ${blockId} at ${x}, ${y}`,
    );
  ...
  return await browser.execute(() => {
    ...
    if (dragStrategy) {
      dragInfo = {
        ...
        allConnectionsCount: dragStrategy.allConnections
          ? dragStrategy.allConnections.length
          : 'N/A',
        connectionHighlighterActive: dragStrategy.connectionHighlighter
          ? dragStrategy.connectionHighlighter.isActive
          : 'N/A',
        highlightedElementsCount: dragStrategy.connectionHighlighter
          ? dragStrategy.connectionHighlighter.highlightedElements?.size
          : 'N/A',
      };
    }
    ...
    return {
      ...
      hasConnectionHighlighter: dragStrategy
        ? !!dragStrategy.connectionHighlighter
        : false,
      highlightingEnabled: dragStrategy
        ? dragStrategy.highlightingEnabled
        : false,
      highlightElementCount: highlights.length,
      highlightElements: Array.from(highlights).map(
...
async function doubleTapBlock(
  ...
  await browser.touchAction([{action: 'tap', x: centerX, y: centerY}]);
...
suite('Connection Highlight Persistence During Preview Tests', function () {
  ...
  test('Connection highlights persist after preview renders', async function () {
    ...
    const initialHighlightCount = await getConnectionHighlightCount(
      this.browser,
    );
    chai.assert.isAbove(
      initialHighlightCount,
      0,
      'Should have initial connection highlights',
    );
    ...
    const highlightCountAfterArrow = await getConnectionHighlightCount(
      this.browser,
    );
    chai.assert.isAbove(
      highlightCountAfterArrow,
      0,
      'Connection highlights should persist after arrow navigation triggering preview',
    );
    ...
    const highlightCountAfterDown = await getConnectionHighlightCount(
      this.browser,
    );
    chai.assert.isAbove(
      highlightCountAfterDown,
      0,
      'Connection highlights should persist after down arrow navigation',
    );
    ...
    const highlightCountAfterUp = await getConnectionHighlightCount(
      this.browser,
    );
    chai.assert.isAbove(
      highlightCountAfterUp,
      0,
      'Connection highlights should persist after up arrow navigation',
    );
    ...
    chai.assert.equal(
      finalHighlightCount,
      0,
      'Connection highlights should be cleared when move mode ends',
    );
  });
  ...
  test('Connection highlights recompute correctly during workspace changes', async function () {
    ...
    const initialHighlights = await getConnectionHighlightPositions(
      this.browser,
    );
    chai.assert.isAbove(
      initialHighlights.length,
      0,
      'Should have initial highlights',
    );
    ...
    for (const key of navigationKeys) {
      ...
      const currentHighlightCount = await getConnectionHighlightCount(
        this.browser,
      );
      chai.assert.isAbove(
        currentHighlightCount,
        0,
        `Connection highlights should persist after ${key} navigation`,
      );

      // Check that highlight elements are properly attached to DOM
      const highlightsAttached = await checkHighlightsAttachedToDOM(
        this.browser,
      );
      chai.assert.isTrue(
        highlightsAttached,
        `All highlights should be properly attached to DOM after ${key} navigation`,
      );
    }
    ...
    chai.assert.equal(
      finalHighlightCount,
      0,
      'All highlights should be cleared when move mode ends',
    );
  });
  ...
  test('Highlights remain visible during insertion marker operations', async function () {
    ...
    const insertionMarkerExists = await this.browser
      .$('.blocklyInsertionMarker')
      .isExisting();
    const highlightCountWithMarker = await getConnectionHighlightCount(
      this.browser,
    );

    // Highlights should persist even when insertion markers are present
    chai.assert.isAbove(
      highlightCountWithMarker,
      0,
      'Connection highlights should persist when insertion markers are shown',
    );
    ...
    if (insertionMarkerExists) {
      ...
      chai.assert.isAbove(
        highlightCountWithMarker,
        0,
        'Highlights should remain visible alongside insertion markers',
      );
    }
    ...
    const highlightCountAfterMoreNavigation = await getConnectionHighlightCount(
      this.browser,
    );
    chai.assert.isAbove(
      highlightCountAfterMoreNavigation,
      0,
      'Highlights should persist through multiple preview operations',
    );
  ...
  afterEach(async function () {
    ...
    await assertNoJavaScriptErrors(
      this.browser,
      'highlight persistence test operations',
    );
  });
});
...
suite('Highlight Persistence During Stack Navigation', function () {
  ...
  test('Double-click middle block, press up arrow, highlights persist until Enter', async function () {
    ...
    chai.assert.isTrue(
      await isDragging(this.browser),
      'Should be in dragging mode after double-click',
    );
    ...
    const highlightsProperlyAttached = await checkHighlightsAttachedToDOM(
      this.browser,
    );
    chai.assert.isTrue(
      highlightsProperlyAttached,
      'All highlights should be properly attached to DOM',
    );
    ...
    chai.assert.isEmpty(
      errorsAfterArrow,
      `Should have no console errors after arrow navigation. Found: ${JSON.stringify(errorsAfterArrow)}`,
    );
    ...
    console.log(
      'Step 7: Verifying highlights are cleared after exiting move mode',
    );
    const finalHighlightCount = await getConnectionHighlightCount(this.browser);
    ...
    chai.assert.isEmpty(
      finalErrors,
      `Should have no console errors at end of test. Found: ${JSON.stringify(finalErrors)}`,
    );
    ...
    chai.assert.isBelow(
      responseTime,
      1000,
      'Browser should remain responsive (no hanging detected)',
    );

    console.log(
      '✅ Test completed successfully - highlights persisted during navigation and cleared on exit',
    );
  });
  ...
  test('Multiple arrow navigations maintain highlight persistence', async function () {
    ...
    chai.assert.equal(
      finalCount,
      0,
      'All highlights should be cleared after escape',
    );
  });
  ...
  afterEach(async function () {
    ...
    await assertNoJavaScriptErrors(
      this.browser,
      'highlight persistence during stack navigation',
    );
  });
});
 ...
 */
async function getConnectionHighlightPositions(
  browser: Browser,
): Promise<Array<{x: number; y: number}>> {
  return await browser.execute(() => {
    const highlights = Array.from(
      document.querySelectorAll('.blocklyPotentialConnection'),
    );
 ...
 */
async function checkHighlightsAttachedToDOM(
  browser: Browser,
): Promise<boolean> {
  return await browser.execute(() => {
    const highlights = Array.from(
