...
suite('Touch-based Click-n-Stick Tests', function () {
  ...
  suite('Double-tap to enter move mode', function () {
    test('Double-tap block enters move mode', async function () {
      ...
      chai.assert.isTrue(
        ...
        'Should be in sticky mode after double-tap',
      );
  ...
  suite('Tap connection to move block', function () {
    test('Double-tap block, tap connection to move block there', async function () {
      ...
      const initialInfo = await getBlockConnectionInfo(
        this.browser,
        'draw_circle_1',
      );
      ...
      const debugInfo = await this.browser.execute(() => {
        ...
        const elementsWithPotential = Array.from(allSvgElements).filter(
          (el: any) => {
      ...
      const connectionHighlight = await this.browser.$(
        '.blocklyPotentialConnection',
      );
      chai.assert.isTrue(
        await connectionHighlight.isExisting(),
        'Should have at least one connection highlight',
      );
      ...
      chai.assert.isFalse(
        ...
        'Should exit move mode after tapping connection',
      );
      chai.assert.isFalse(
        await isInStickyMode(this.browser),
        'Should exit sticky mode after tapping connection',
      );
      ...
      const finalInfo = await getBlockConnectionInfo(
        this.browser,
        'draw_circle_1',
      );
    ...
    test('Connection highlights persist during touch move gesture', async function () {
      ...
      const highlightCountAfterMove = await getConnectionHighlightCount(
        this.browser,
      );
      chai.assert.isAbove(
        highlightCountAfterMove,
        0,
        'Connection highlights should persist during touch move',
      );
  ...
  suite('Touch move and release', function () {
    test('Double-tap, tap workspace drops block at that location disconnected', async function () {
      ...
      const initialConnections = await getBlockConnectionInfo(
        this.browser,
        'draw_circle_1',
      );
      ...
      chai.assert.isFalse(
        await isDragging(this.browser),
        'Should exit move mode after tap',
      );
      chai.assert.isFalse(
        await isInStickyMode(this.browser),
        'Should exit sticky mode after tap',
      );
      ...
      const finalConnections = await getBlockConnectionInfo(
        this.browser,
        'draw_circle_1',
      );
      chai.assert.isNull(
  ...
  suite('Connection highlights cleanup with touch', function () {
    test('Connection highlights are cleared after tap-to-connect', async function () {
      ...
      chai.assert.isAbove(
        highlightCount,
        0,
        'Should have connection highlights',
      );
      ...
      const finalHighlightCount = await getConnectionHighlightCount(
        this.browser,
      );
      chai.assert.equal(
        finalHighlightCount,
        0,
        'Connection highlights should be cleared after tap-to-connect',
      );
    });
    ...
    test('Connection highlights are cleared after tap-to-release on workspace', async function () {
      ...
      chai.assert.equal(
        finalHighlightCount,
        0,
        'Connection highlights should be cleared after tap-to-release',
      );
    });
  });
  ...
  suite(
    'Bug: Single tap selection broken after touch click-and-stick',
    function () {
